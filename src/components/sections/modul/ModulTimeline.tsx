...
/** The marker printed before each week, like a checklist in a terminal. */
...
/**
 * The whole semester as one rail: finished weeks filled in, this week lit
 * in the accent and marked `aria-current`, the rest hollow and dimmed with
 * their release day. The status is always spelled out in text as well —
 * colour and the node's fill are only the second way of saying it.
 */
export function ModulTimeline({ modules }: ModulTimelineProps) {
  return (
    <ol className="border-l-2 border-line">
      {modules.map((modul) => {
        const current = modul.status === 'berjalan'
        const locked = modul.status === 'terkunci'

        return (
          <li
            key={modul.id}
            id={`minggu-${pad2(modul.minggu)}`}
          ...
          >
            <span
              aria-hidden
              className={cn(
                'absolute top-[1.85rem] -left-[8px] block h-3.5 w-3.5 border-2 border-line',
                current ? 'bg-accent' : locked ? 'bg-canvas' : 'bg-line',
              )}
            />
            ...
            <div className="flex items-baseline gap-3 sm:flex-col sm:gap-1">
              <p
                className={cn(
                  ...
                  locked ? 'text-dim' : 'text-accent-fg',
                )}
              >
                minggu {pad2(modul.minggu)}
              </p>
              <p className="text-[11px] leading-5 text-dim tabular-nums">{modulRange(modul)}</p>
            </div>
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
                ...
                {current ? (
                  ...
                  <span className="sr-only">({STATUS_TEXT[modul.status]})</span>
                )}
              </div>
              ...
              <p className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-[11px] leading-5 text-dim">
                <span>
                  tentor pj: <span className="text-muted">{modul.tentorPj.join(', ')}</span>
                </span>
                <span aria-hidden>·</span>
                <span>
                  koordinator: <span className="text-muted">{modul.koordinator}</span>
                </span>
                <span aria-hidden>·</span>
                {locked ? (
                  <span>dibuka {modulRelease(modul)}</span>
                ) : modul.berkasUrl ? (
                  <a
                    href={modul.berkasUrl}
                    target="_blank"
