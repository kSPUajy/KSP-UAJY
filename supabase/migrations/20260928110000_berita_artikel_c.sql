-- The first set of news posts reported events that never happened. Replace
-- them with articles about C itself (facts and beginner tips), keep the
-- registration notice and the scanf tutorial with their made-up claims
-- removed, and add the two new categories those articles use.

alter table public.news_posts drop constraint news_posts_kategori_check;
alter table public.news_posts add constraint news_posts_kategori_check
  check (kategori in ('pengumuman', 'fakta', 'tips', 'tutorial', 'liputan', 'prestasi'));

delete from public.news_posts where id in ('n-02', 'n-03', 'n-04', 'n-05', 'n-07', 'n-08');
...
Kompilasi dengan `-Wall` supaya compiler memperingatkan hal-hal yang mencurigakan,
misalnya variabel yang dibuat tapi tidak pernah dipakai:
...
Hasilnya 5: empat huruf ditambah satu karakter baris baru.$ksp$, array[$ksp$tips$ksp$, $ksp$printf$ksp$, $ksp$format$ksp$]::text[]),
  ($ksp$n-11$ksp$, $ksp$membaca-pesan-error-compiler$ksp$, $ksp$Membaca Pesan Error Compiler Tanpa Panik$ksp$, $ksp$2026-09-12$ksp$, $ksp$tips$ksp$, $ksp$https://picsum.photos/seed/ksp-artikel-error/1200/675$ksp$, $ksp$Pesan error itu teman: compiler memberi tahu letak masalahnya, bahkan sering menebak perbaikannya. Begini cara membacanya.$ksp$, $ksp$Tim KSP$ksp$, $ksp$Pertama kali melihat layar penuh pesan error itu menakutkan. Tapi pesan error
sebenarnya adalah teman: compiler sedang memberi tahu di mana masalahnya, lengkap
dengan saran perbaikannya. Kamu hanya perlu tahu cara membacanya.

## Anatomi satu pesan error

Misalnya kamu lupa titik koma:
...
gcc akan menjawab seperti ini:

```
galat.c:4:27: error: expected ';' before 'return'
    4 |     printf("%d\n", jumlah)
      |                           ^
      |                           ;
```
...
- `galat.c` adalah nama file.
- `4` adalah nomor baris, dan `27` adalah posisi kolomnya.
- `error` artinya program tidak bisa dijalankan sampai ini diperbaiki.
- Sisanya adalah penjelasan: compiler mengharapkan `;` sebelum `return`.

Di bawahnya, compiler bahkan menunjukkan letak persisnya dengan tanda `^` dan
menuliskan karakter yang kurang.

## Compiler sering menebak maksudmu

Kalau kamu salah ketik nama variabel:

```
galat.c:4:20: error: 'jumlh' undeclared (first use in this function); did you mean 'jumlah'?
```

`undeclared` artinya nama itu belum pernah dibuat. Bagian `did you mean` adalah
tebakan compiler, dan biasanya tebakannya benar.

## Error dan warning itu berbeda

**Error** membuat program gagal dikompilasi. **Warning** tidak menghentikan
kompilasi, tapi hampir selalu menandakan ada yang salah. Contoh paling klasik
adalah lupa tanda `&` di `scanf`:
...
Karena itu, biasakan kompilasi dengan `-Wall` supaya semua warning penting
ditampilkan:
...
2. **Lihat juga baris sebelumnya.** Kadang kesalahannya ada di baris sebelum
   yang ditunjuk, misalnya titik koma atau kurung kurawal yang lupa ditutup.
